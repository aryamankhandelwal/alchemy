// Per-stage KPI schema — single source of truth for the KPI tracker, the
// card health dots, and the AI system prompt. Mirrors the New Horizons
// memo (Stage 1/2/3 KPI tables, p.5–6).

const pct = v => `${Math.round(v * 100)}%`
const x = v => `${v.toFixed(1)}x`
const months = v => `${v} months`
const weeks = v => `${v} weeks`

const enumEval = (map) => (v) => map[v] ?? 'Proceed'
...
export const KPI_SCHEMA = {
  Evaluation: {
    demandSignal: {
      ...
      evaluate: v => v < 0.05 ? 'Kill' : v < 0.15 ? 'Proceed' : 'Prioritise'
    },
    problemSeverity: {
    ...
    speedToMvp: {
      ...
      evaluate: v => v > 26 ? 'Kill' : v > 6 ? 'Proceed' : 'Prioritise'
    }
  },
  ...
  Pilot: {
    activationRate: {
      ...
      evaluate: v => v < 0.20 ? 'Kill' : v < 0.40 ? 'Proceed' : 'Prioritise'
    },
    conversionRate: {
    ...
    ltvCac: {
      ...
      evaluate: v => v < 1.0 ? 'Kill' : v < 2.0 ? 'Proceed' : 'Prioritise'
    },
    riskSignals: {
      label: 'Risk Signals',
  ...
  Scale: {
    ltvCac: {
      ...
      evaluate: v => v < 1.5 ? 'Kill' : v < 2.5 ? 'Proceed' : 'Prioritise'
    },
    contributionMargin: {
      ...
      formatValue: v => v,
      thresholds: 'Negative kill · Approaching breakeven proceed · Positive + improving prioritise',
      evaluate: enumEval({ Negative: 'Kill', 'Approaching breakeven': 'Proceed', 'Positive + improving': 'Prioritise' })
    },
    paybackMonths: {
      ...
      evaluate: v => v > 18 ? 'Kill' : v > 9 ? 'Proceed' : 'Prioritise'
    },
    riskScalability: {
    ...
    operationalScalability: {
      ...
      evaluate: enumEval({ Breaks: 'Kill', 'Needs optimisation': 'Proceed', 'Scales cleanly': 'Prioritise' })
    },
    strategicFit: {
      label: 'Strategic Fit',
