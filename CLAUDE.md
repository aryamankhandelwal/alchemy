...
2. `py server/pyserver.py` — **the standard agent fallback.** A no-node Python drop-in on the same port (:5173) that serves the prebuilt `dist/` SPA, reproduces every `/api/*` route (chat / enrich / research / bets CRUD via Gemini REST + MongoDB Atlas), and retries Gemini 429/500/503 with backoff. Run it in the background, then verify with `curl http://localhost:5173/api/bets`. Doc generation (`/api/bets/:id/generate-doc`, `/api/artifacts/:id/docx`) and artifact routes exist **only** in pyserver and need `py -m pip install python-docx reportlab`. Generated PDFs store their content JSON on the artifact (`gen` field) so the chat's `docUpdate` patch key can revise + replace them, and docx conversion never re-calls Gemini.
...
├── index.html
├── .env.example
├── seed/                           default portfolio, inserted when the DB is empty
│   ├── bets.json                   bets snapshot (pyserver seeder reads this)
│   ├── artifacts.json              artifact metadata manifest
│   └── files/                      artifact binaries (docx/pdf)
├── scripts/
│   └── snapshot-seed.py            regenerate seed/ + src/data/bets.ts from the live DB
├── server/
...
│   ├── score.ts                    /api/score/:id — AI rates RICE dimensions (1-10); score = 100×(R·I·C·(11−E)/10⁴)^0.25, computed server-side
│   ├── granolaExtract.ts           /api/granola/extract — proposals from a pasted meeting transcript
│   ├── docgen.py                   Memo/PRD generation: Gemini content JSON → PDF (reportlab) + on-demand docx (python-docx)
...
└── src/
    ...
    │   └── granola.ts              GranolaProposal / GranolaExtractResult (Granola Sync wire types)
    ├── data/
    │   └── bets.ts                 seed bets — generated by scripts/snapshot-seed.py (inserted only when the DB is empty)
    ├── lib/
    ...
    └── components/
        ...
        ├── GranolaSyncModal.tsx    paste a meeting transcript → review/apply AI-extracted change proposals
...
Every patch is diffed against the previous bet by `history.ts` and appended to `bet.history` as a `HistoryEntry`. Board drags are recorded the same way under source `'drag'`; Granola Sync applies under source `'granola'`.

### Granola Sync (transcript → proposals)

The **Granola Sync** header button opens `GranolaSyncModal`: paste a meeting transcript (from Granola — no API integration yet, v0 is paste-based), the frontend POSTs `{ transcript, meetingTitle? }` to `/api/granola/extract`, and the server (`server/granolaExtract.ts` / pyserver) makes one Gemini call that returns:

```ts
{ meetingTitle: string, proposals: GranolaProposal[] }   // src/types/granola.ts
```

Each proposal is either an **edit** (a normal dot-path `patch` against an existing bet — same contract as chat, shared via `PATCH_REFERENCE` exported from `src/lib/systemPrompt.ts` and mirrored in pyserver) or a **new_bet** skeleton, plus a verbatim evidence quote. The server verifies each quote with a normalized substring match against the transcript (`quoteVerified`); unverified quotes get an amber flag in the UI. The user dismisses unwanted cards and applies the rest: edits go through `api.patchBet` with source `'granola'` and note `From meeting: <title>`, new bets reuse the Add-bet flow (create → enrich → KpiReviewModal → background research). Nothing is persisted until Apply.
